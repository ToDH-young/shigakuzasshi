...
# 結果を出力するファイルを指定
# result_file = open('./result0626.txt', mode='a', encoding='utf-8')
result_file = open(sys.argv[1], mode='a', encoding='utf-8')
...
for publisher, publisher_url in publisher_dict.items():
    ...
    for book_url in book_urls:
        ...
        raw_title = book_dict['@graph'][0]['dc:title'][0]['@value']
        title, subtitle = modify_title_data(raw_title)
        # これでタイトルの文字列を取得できた

        raw_author = book_dict['@graph'][0]
        author = modify_author_data(raw_author)
        # 著者を取得

        raw_date = book_dict['@graph'][0]['dc:date']
        date = modify_date_data(raw_date)
        # 発行年月を取得

        raw_isbn = book_dict['@graph'][0]
        ...
        t = '\t'
        result = f'""{t}""{t}{author}{t}""{t}{title}{t}{subtitle}{t}{publisher}{t}{date}\
                    {t}""{t}""{t}""{t}""{t}""{t}{isbn}'
        if repetition == 0:
